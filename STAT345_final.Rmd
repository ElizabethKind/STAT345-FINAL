...
chest <- function() {
  ...
  if (players[[j]]$location %in% chest_positions) {
    if (chest_card == 1) {
      ...
      players[[j]]$money <- players[[j]]$money + 200 # Bank error in your favor. Collect $200
    } else if (chest_card == 3) {
      players[[j]]$money <- players[[j]]$money - 50 # Doctor’s fee. Pay $50
      free_parking_fund <- free_parking_fund + 50
    } else if (chest_card == 4) {
      players[[j]]$money <- players[[j]]$money + 50 # From sale of stock you get $50
      ...
      players[[j]]$money <- players[[j]]$money + 100 # Holiday fund matures. Receive $100
    } else if (chest_card == 8) {
      players[[j]]$money <- players[[j]]$money + 20 # Income tax refund. Collect $20
    } else if (chest_card == 9) {
      players[[j]]$money <- players[[j]]$money + 10 * (length(players) - 1) # It is your birthday. Collect $10 from every player
        for (k in 1:length(players)) {
          if (k != j) {
      ...
      players[[j]]$money <- players[[j]]$money + 100 # Life insurance matures. Collect $100
    } else if (chest_card == 11) {
      players[[j]]$money <- players[[j]]$money - 100 # Pay hospital fees of $100
      free_parking_fund <- free_parking_fund + 100
    } else if (chest_card == 12) {
      players[[j]]$money <- players[[j]]$money - 50 # Pay school fees of $50
      free_parking_fund <- free_parking_fund + 50
    } else if (chest_card == 13) {
      players[[j]]$money <- players[[j]]$money - 25 # Receive $25 consultancy fee
      free_parking_fund <- free_parking_fund + 25
    } else if (chest_card == 14) {
      players[[j]]$money <- players[[j]]$money + 10 # You have won second prize in a beauty contest. Collect $10
    } else if (chest_card == 15) {
      players[[j]]$money <- players[[j]]$money + 100 # You inherit $100
    }
      # Card 16 is paying for houses and hotels -- doesn't apply here
  }
}
...
free_parking <- function(){
  ...
  players[[j]]$money <- players[[j]]$money + free_parking_fund
  free_parking_fund <- 0  # Reset the fund
  }
}
```
### Chance function (Lucy's variables)
```{r}
chance <- function() {
  card <- sample(1:16, 1)
  if (card == 1) {
...
while (# at least 2 players are still in) {
  ...
  for (j in 1:4) {
    if (players[[j]]$in != FALSE) {
      ...
      # If players account is negative then they are out
    }  
  }
}
```
