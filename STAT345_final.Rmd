...
# Chance function
chance_positions <- c(8, 23, 37)
chance <- function() {
  card <- sample(1:16, 1)
if (players[[j]]$location %in% chance_positions) {
  if (card == 1) {
...
chance <- function() {
 ...
 if (players[[j]]$location %in% chance_positions) {
  ...
  if (players[[j]]$location < players[[j]]$location && card %in% c(3,4,10)) {
    players[[j]]$money <- players[[j]]$money + 200 # Passing Go for cards 3, 4, and 10
      ...
      players[[j]]$get_out_jail == TRUE # Get out of jail free card
    }
    else if (card == 13) {
      players[[j]]$money <- players[[j]]$money - 15 # Speeding fine $15
      properties_info$free_parking <- properties_info$free_parking + 15
    }
    else if (card == 14) {
...
chest <- function() {
  ...
  if (players[[j]]$location %in% chest_positions) {
    if (chest_card == 1) {
      ...
      players[[j]]$money <- players[[j]]$money + 10*players[[j]]
